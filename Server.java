...
public class Server extends Thread {
        ...
        while ((!Network.getClientConnectionStatus().equals("disconnected")))
        {
            synchronized (Server.class) // synchronize server class
            {
                if (!Network.getInBufferStatus().equals("empty"))
                {
                    ...
                    if (accIndex == -1)
                        System.out.println("Error" + trans.getAccountNumber().toString());
    ...
    public void run() {
        ...
        // If we're on thread 1, set status to run, if we're on thread 2, set status to run.
      ...
      // If we're on thread 1, we idle, if we're on thread 2, we idle.
        if (this.getServerThreadId().equals("1")) {
        ...
        if (serverThreadRunningStatus1.equals("idle") && serverThreadRunningStatus2.equals("idle"))
        {
